...
void dfs(long long x,long long mn){long long i,sz=al[x].size(),l,p;vtd[x]=1;if(x==n+2){z-=mn;cr=mn;r0=1;return;}
	for(i=0;i<sz;i++){l=al[x][i].fr;p=al[x][i].sc;if(!vtd[l]&&wg[p]){dfs(l,min(mn,wg[p]));if(r0){wg[p]-=cr;wg[p^1]+=cr;return;}}}}
